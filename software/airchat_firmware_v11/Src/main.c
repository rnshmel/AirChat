...
void RxUartByteFunc(uint8_t UartRxBuffer[], uint8_t* UartRxByte, uint8_t* UartRxBufferPos,
		uint8_t* UartRxNewFlag, UART_HandleTypeDef huart2)
{
	//uint8_t UartRxStatus = 0; // status to determine HAL exit code
	//UartRxStatus = HAL_UART_Receive(&huart2,UartRxByte,1,0);
	if (!(HAL_UART_Receive(&huart2,UartRxByte,1,0)))
	//if (UartRxStatus == 0) // did receive a single byte
	{
		UartRxBuffer[*UartRxBufferPos] = *UartRxByte; // add to buffer
		*UartRxBufferPos += 1; // advance buffer position variable
		//UartRxBufferPosition %= MAX_MSG_LENGTH; // should never get here, but just in case, don't overflow!
		*UartRxNewFlag = 1; // set the flag to show we have a new UART signal
	}
}
...
int main(void)
{
  ...
  {
	  /* -------- RX UART SECTION -------- */
	  RxUartByteFunc(UartRxBuffer, &UartRxByte, &UartRxBufferPos, &UartRxNewFlag, huart1);

	  // if the UART RX "new flag" is set, check for end of line '\n'
	  if (UartRxNewFlag == 1)
	  {
		  UartRxNewFlag = 0; // set flag to 0
		  if (UartRxBuffer[UartRxBufferPos-1] == 255)
		  {
			  // newline, end of stream, check where data should go
	  ...
	  if (SpiTxFlag == 1)
	  {
		  ...
		  if (backoffFlag == 0)
		  {
			  ...
			  {
				  cc1101_TX_packet(hspi1, SpiTxBuffer);
	  ...
	  /* -------- RX RF SECTION -------- */
	  // check if GDO2 is set (SFD check)
	  if (HAL_GPIO_ReadPin(GDO2_GPIO_Port, GDO2_Pin) && (RxStateFlag == 0))
	  {
		  // SFD pass, RXFIFO will now start filling, set RX state flag to 1
		  HAL_GPIO_WritePin(RLED_GPIO_Port, RLED_Pin, GPIO_PIN_SET); // turn on RX LED
	  ...
	  if (UartTxFlag == 1)
	  {
		  ...
		  {
			  HAL_UART_Transmit(&huart1, &UartTxBuffer[UartTxCount], 1, 10000);
			  UartTxCount += 1;
		  }
	  }

	  //count = __HAL_TIM_GET_COUNTER(&htim16); // end timer
